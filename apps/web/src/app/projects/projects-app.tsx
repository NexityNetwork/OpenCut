...
import { VaultSection } from "@/projects/vault-section";
...
function ProjectItem({
	...
	const gridContent = (
		<Card className="bg-transparent overflow-visible border-none p-0 shadow-none">
			<div className="bg-muted relative aspect-video overflow-hidden rounded-xl border border-border/60 ring-1 ring-inset ring-white/5 shadow-sm transition-all duration-200 group-hover:border-border group-hover:shadow-xl group-hover:shadow-black/30">
				...
				{durationLabel && (
					<div className="absolute bottom-2 right-2 bg-black/60 text-white text-xs font-semibold px-2 py-1 rounded-md backdrop-blur-sm">
						{durationLabel}
					</div>
				)}
			</div>

			<CardContent className="flex flex-col gap-1 px-0.5 pt-3">
				<h3 className="group-hover:text-foreground line-clamp-1 text-sm leading-snug font-medium transition-colors">
					{project.name}
				</h3>
	...
	return (
		<div className="px-4 xs:grid-cols-2 grid grid-cols-1 gap-6 sm:grid-cols-3 lg:grid-cols-4">
			{skeletonIds.map((skeletonId) => (
				<Card
					key={skeletonId}
					className="bg-transparent overflow-visible border-none p-0 shadow-none"
				>
					<div className="bg-muted relative aspect-video overflow-hidden rounded-xl border border-border/60">
						<div className="absolute inset-0">
					...
					<CardContent className="flex flex-col gap-1 px-0.5 pt-3">
						...
						<div className="text-muted-foreground flex items-center gap-1.5">
							...
							<Skeleton className="bg-muted/50 h-3.5 w-24" />
						</div>
					</CardContent>
				</Card>
			))}
