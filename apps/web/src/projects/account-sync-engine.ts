// Client-only sync engine. Kept out of the AccountSync component's module graph
// so it is never evaluated during SSR (it touches IndexedDB/OPFS via
// storageService). Loaded via dynamic import inside client effects.
...
export async function reconcile(owner: string) {
	const [local, remote] = await Promise.all([
