...
import { renderShaderFrame } from "@/shaders/render";
...
async function resolveStickerNode({
	...
	if (node.params.stickerId.startsWith("lottie:")) {
		return resolveLottieStickerNode({ node, context });
	}

	if (node.params.stickerId.startsWith("shader:")) {
		return resolveShaderStickerNode({ node, context });
	}
...
async function resolveLottieStickerNode({
	node,
	context,
}: {
	node: StickerNode;
	...
	const { providerValue } = parseStickerId({ stickerId: node.params.stickerId });
	const data = await getLottieData(`/lottie/${providerValue}.json`).catch(
		() => null,
	);
	if (!data) return null;

	const { width: sourceWidth, height: sourceHeight } = getLottieRenderSize(data);
	const visualState = resolveVisualState({
		params: node.params,
		context,
		sourceWidth,
	...
	const timeSeconds = mediaTimeToSeconds({
		time: roundMediaTime({ time: context.time - node.params.timeOffset }),
	});
	const canvas = renderLottieFrame({ owner: node, data, timeSeconds });
...
// Max rasterized dimension for shader backgrounds; aspect matches the project
// so the result covers the frame exactly (containScale === 1).
const MAX_SHADER_DIM = 1280;

async function resolveShaderStickerNode({
	node,
	context,
}: {
	node: StickerNode;
	...
	const { providerValue: shaderId } = parseStickerId({
		stickerId: node.params.stickerId,
	});

	const targetWidth = Math.max(1, context.renderer.width);
	const targetHeight = Math.max(1, context.renderer.height);
	const scale = Math.min(
		1,
		MAX_SHADER_DIM / Math.max(targetWidth, targetHeight),
	);
	const sourceWidth = Math.max(1, Math.round(targetWidth * scale));
	const sourceHeight = Math.max(1, Math.round(targetHeight * scale));
	...
	const canvas = renderShaderFrame({
		owner: node,
		shaderId,
		timeSeconds,
		width: sourceWidth,
