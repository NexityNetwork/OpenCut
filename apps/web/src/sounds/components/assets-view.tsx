...
import { VoiceoverView } from "@/sounds/components/voiceover-view";
import { LocalAudioLibrary } from "@/sounds/components/local-audio-library";
	...
	return (
		<div className="flex h-full flex-col">
			<Tabs defaultValue="sound-effects" className="flex h-full flex-col">
				...
				>
					<LocalAudioLibrary
						manifestUrl="/audio/sfx/manifest.json"
				...
				<TabsContent
					...
					className="mt-0 flex min-h-0 flex-1 flex-col"
				>
					<LocalAudioLibrary source="saved" searchPlaceholder="Search saved" />
				</TabsContent>
			</Tabs>
		</div>
	);
...
function SoundEffectsView() {
	...
	const {
		openFilePicker: openSoundImport,
		fileInputProps: soundImportInputProps,
	} = useMediaImport({ accept: "audio/*" });
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center gap-3">
				...
				<input {...soundImportInputProps} />
				<Button
					variant="text"
					size="icon"
					onClick={openSoundImport}
					...
					<div className="flex flex-col gap-4">
						...
						{!isLoading && !isSearching && displayedSounds.length === 0 && (
							<div className="text-muted-foreground text-sm">
								{searchQuery
									? "No sounds found"
									: "No sounds available — use Import to add your own audio"}
							</div>
						)}
						{isLoadingMore && (
