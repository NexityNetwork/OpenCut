...
function StickersContentView() {
	...
	if (viewMode === "search") {
		...
		if (selectedCategory !== "all" && searchQuery) {
			return <EmptyView message={`No presets found for "${searchQuery}"`} />;
		}
	}
				...
				message={
					viewMode === "search"
						...
						: selectedCategory === "all"
							...
							: `No presets available in ${categoryLabel.toLowerCase()} yet.`
				}
			/>
		);
	}
	...
	const preview = (
		<div
			className={cn(
				...
				item.provider === "shader" ? "p-0" : "p-3",
			)}
		>
			{item.provider === "shader" ? (
				<ShaderPreview
					shaderId={item.id.split(":").slice(1).join(":")}
					className="size-full object-cover"
				/>
			) : item.provider === "lottie" ? (
				<LottiePreview
					url={item.previewUrl}
					className="flex size-full items-center justify-center [&_svg]:size-full [&_svg]:object-contain"
				/>
			) : hasImageError ? (
