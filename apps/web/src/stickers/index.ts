...
export async function resolveStickerIntrinsicSize({
	...
	// Shaders are full-frame backgrounds; report a 16:9 intrinsic size. Actual
	// on-canvas sizing is computed from the project resolution at render time.
	if (stickerId.startsWith("shader:")) {
		return { width: 1920, height: 1080 };
	}

	// Lottie payloads are JSON, not images, so read the canvas size from the
	// animation data rather than loading it through an <img>.
	if (stickerId.startsWith("lottie:")) {
		try {
			const { lottieUrlFromKey, getLottieData } = await import("@/lottie/render");
			const { providerValue } = parseStickerId({ stickerId });
			const data = await getLottieData(lottieUrlFromKey(providerValue));
