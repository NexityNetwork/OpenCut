...
import { ShaderPreview } from "@/shaders/preview";
...
function StickerElementContent({
	...
	// Shader backgrounds render via WebGL, not an image URL.
	if (element.stickerId.startsWith("shader:")) {
		return (
			<div className="flex size-full items-center gap-2 pl-2">
				<ShaderPreview
					shaderId={parseStickerId({ stickerId: element.stickerId }).providerValue}
	...
	// Lottie stickers are JSON, not images — render the animated SVG preview
	// instead of an <img>, which would show a broken-image icon.
	if (element.stickerId.startsWith("lottie:")) {
		return (
			<div className="flex size-full items-center gap-2 pl-2">
				<LottiePreview
					url={lottieUrlFromKey(
						parseStickerId({ stickerId: element.stickerId }).providerValue,
					)}
		...
		<>
			<div
				...
				style={{
					...
					// Fit the thumbnail to track height at its own aspect (auto width);
					// a fixed 16:9 tile stretched portrait/square frames.
