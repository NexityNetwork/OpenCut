...
async function composeSlide(slide, opts) {
  ...
  for (let i = 1; i <= 9; i++) {
    if (html.includes(`{{iconSvg${i}}}`)) {
      fills[`iconSvg${i}`] = resolveIcon(
        ...
        // Mid-grey, not near-black: an unknown brand on a dark template used
        // to render #1d1d1d on #08090A — an invisible icon that still passes
        // every render check. This tone is legible on both cream and black.
        onDark(slide, slide.slots[`iconColor${i}`] || autoBrandColor(slide.slots[`iconSlug${i}`]) || "#8A8F98"),
      );
    }
  }
  ...
  // Safety net: planner sometimes emits Markdown emphasis (*word* / **word** /
  // _word_) for body fields, which render as literal characters. Strip them on
  // plain string slots before substitution. Keep raw {{iconSvg<N>}} (HTML)
  // untouched.
  for (const key of Object.keys(fills)) {
    if (/^iconSvg/i.test(key)) continue;
    const v = fills[key];
...
const BRAND_FALLBACK = {
  ...
  clearbit: { color: "#3B82F6", glyph: "Cb" },
  apolloio: { color: "#311C87", glyph: "AP" },
  outreach: { color: "#5951FF", glyph: "Ou" },
  lemlist: { color: "#3E63DD", glyph: "Ll" },
  instantly: { color: "#4F46E5", glyph: "In" },
  // Data + analytics
...
/**
 * Brand colors are authored for light backgrounds. Notion, GitHub, Vercel,
 * Apple and friends are officially #000000 — on a dark template they render
 * as an invisible icon that still passes every render and QA check. A slide
 * opts in with `iconTheme: "dark"` and any too-dark brand color is lifted to
 * the template's ink instead.
 */
function onDark(slide, color) {
  if (String(slide.slots?.iconTheme || "").toLowerCase() !== "dark") return color;
  const m = /^#?([0-9a-f]{6})$/i.exec(String(color).trim());
  if (!m) return color;
  const n = parseInt(m[1], 16);
  const lum = (((n >> 16) & 255) * 0.299 + ((n >> 8) & 255) * 0.587 + (n & 255) * 0.114) / 255;
  return lum < 0.22 ? (slide.slots?.iconInk || "#F2F3F5") : color;
}
...
function resolveIcon(slug, svg, color) {
  ...
  if (slug) {
    ...
    // No simple-icons match → clean letter monogram in the brand color
    // (first letter of the tool name) instead of a blank white chip.
    const letter = ((displayNameFromSlug(slug) || normalized || "?").trim().charAt(0) || "?").toUpperCase();
    return `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><text x="12" y="17.5" font-family="Inter, system-ui, sans-serif" font-size="18" font-weight="800" letter-spacing="-0.5" fill="${color}" text-anchor="middle">${letter}</text></svg>`;
  }
  return "";
}
