...
THE CARD'S BOTTOM IS NEVER LOOKED FOR. Every crop is the same height and the
capture is cut off, the reference's own move, and the one thing that gives N
frames one footprint.
"""
import glob
import os
import sys
...
def main(deck, src, out):
    ...
    files = [f for f in files if not f.endswith("01.jpg")][:len(cfg["names"])]
    if len(files) != len(cfg["names"]):
        sys.exit(f"expected {len(cfg['names'])} tool frames in {src}, found {len(files)}")
    ...
    for nm, f in zip(cfg["names"], files):
        ...
        im.crop((x0, top + 3, x1, top + 3 + cfg["card_h"])).save(f"{out}/shot-{nm}.png")
        print(f"  {nm:11} icon {box[2]-box[0]:3}px   card {x1-x0}x{cfg['card_h']} at y{top+3}")
